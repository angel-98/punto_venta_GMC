...
function addDetail(id,producto,concentracion,adicional,precio,cantidad){
	...
	if (id!="") {
		...
		'<td class="text-center"><div class="btn-group"><button type="button" class="btn btn-default border-danger btn-sm" href="#" onclick="deleteDetail('+cont+')"><i class="fa fa-times fa-xs"></i></button></div></td>'+
        ...
        '<td class="text-center"><input class="form-control text_fondo" type="text" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" onchange="modifySubtotals()" name="precio_venta[]" id="precio_venta[]" step="0.01" value="'+price+'" readonly></td>'+
		'<td class="text-center"><div class="input-group"><input class="form-control text_fondo" type="text" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" onchange="modifySubtotals()" name="descuento[]" step="0.01" value="'+descuento+'"><div class="input-group-append"><span class="btn btn-default"><i class="fa fa-percent fa-xs"></i></span></div></div></td>'+
...
function addDetailcart(id,producto,concentracion,adicional,precio,cantidad,data){
	...
	if (id!="") {
		var subtotal=cantidad*price;
		var fila='<tr class="filas" id="fila'+cont+'">'+
		`<td class="text-center"><div class="btn-group"><button type="button" name='${JSON.stringify(data)}' class="btn btn-default border-danger btn-sm" href="#" onclick="deleteDetailCart(${cont}, this)"><i class="fa fa-times fa-xs"></i></button></div></td>`+
        '<td><input type="hidden" name="prod_id[]" value="'+id+'">'+producto+' '+concentracion+' '+adicional+'</td>'+
		'<td class="text-center"><input class="form-control text_fondo cantidad" type="number" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" min="1" max="'+stock+'" onchange="ver_stock(this.value,'+stock+')" name="cantidad[]" id="cantidad[]" value="'+numero_cantidad+'"></td>'+
        '<td class="text-center"><input class="form-control text_fondo" type="text" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" onchange="modifySubtotals()" name="precio_venta[]" id="precio_venta[]" step="1" value="'+price+'" readonly></td>'+
        '<td class="text-center"><div class="input-group"><input class="form-control text_fondo" type="text" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" onchange="modifySubtotals()" name="descuento[]" step="0.01" value="'+descuento+'"><div class="input-group-append"><span class="btn btn-default"><i class="fa fa-percent fa-xs"></i></span></div></div></td>'+
		'<td class="text-center">'+simbolo+'<span id="subtotal'+cont+'" name="subtotal">'+subtotal+'</span></td>'+
		'</tr>';
		cont++;
...
function modifySubtotals(){
	...
	for (var i = 0; i < cant.length; i++) {
		...
		total_actual = value=inpV.value*inpP.value;
		descuento_actual = (des.value*total_actual)/100;
		inpS.value=total_actual-descuento_actual;
...
$(document).on('submit', '#form', function(e){
	...
	var fecha = $('#fecha').val();
	var tabla = $('#tableDetails').html();
	var tipo_comprobante = $('#tipo_comprobante').val();
	var serie_comprobante = $('#serie_comprobante').val();
	var num_comprobante = $('#num_comprobante').val();
	var impuesto = $('#impuesto').val();
	if(cliente != '0' && fecha != '' && tipo_comprobante != '' && serie_comprobante != '' && num_comprobante !='' && impuesto != '' && tabla != ''){
		...
		Swal.fire({
			...
			text: "Al realizar la venta,no se podra efectuar ningún cambio...",
			icon: 'warning',
			...
			if (result.value) {
				...
				$.ajax({
					...
					success: function (data) {
						...
						list_produc();
						if(json['comprobante']=="Ticket"){
							
							window.open(base_url+'/Reports/ticket.php?id='+json['id'],
							'win2',
							...
							window.open(base_url+'/Reports/voucher.php?id='+json['id'],
							'win2',
							'status=yes,toolbar=yes,scrollbars=yes,titlebar=yes,menubar=yes,'+
							'resizable=yes,width=800,height=800,directories=no,location=no'+
							'fullscreen=yes');
...
$(document).on("click", "#btnGuardarC", function () {
	...
	if(nombre != '' && dni != '' && direccion != '' && celular != '' && correo != ''){
		...
		$.ajax({
			type: "POST",
			url:base_url+'/Ajax/clienteAjax.php?op=saveandedit',
			data: formdata,
			cache: false,
			contentType: false,
			processData: false,
			success: function (data) {
				...
				Swal.fire({
					title: 'Registrado',
					text: 'Cliente registrato, Correctamente!..',
					icon: 'success',
					showConfirmButton: false,
					timer: 1500
				});
