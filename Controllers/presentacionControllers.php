    ...
    class presentacionControllers extends presentacionModels{
        ...
        public function list_present_controller(){
            ...
            while ($reg=$query->fetch()) {
                $data[]=array(
                    "0"=>
                    '<div class="btn-group"><button class="btn btn-default border-info btn-sm edit"   title="Actualizar" id="'.mainModel::encryption($reg['present_id']).'">
                    <i class="fa fa-edit fa-xs"></i>
                    </button>'.' '.' 
                    <button type="button" class="btn btn-default border-danger btn-sm delete"   title="Eliminar" id="'.mainModel::encryption($reg['present_id']).'">
                    <i class="fa fa-trash fa-xs"></i>
                    </button></div>',
                    "1"=>$reg['present_nombre']
                );
                $i++; 
            }
            $results=array(
