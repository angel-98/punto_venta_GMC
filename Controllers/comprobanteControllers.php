    ...
    class comprobanteControllers extends comprobanteModels{
        ...
        public function add_voucher_controller(){
            ...
            if ($query1->rowCount()>=1) {
                ...
                $nuevo_nombre = ucwords(strtolower($nombre));
                $nueva_letraSerie = strtoupper($letraSerie);
                $dataVoucher=[
                    "Nombre"=>$nuevo_nombre,
                    "LetraSerie"=>$nueva_letraSerie,
                    "Serie"=>$serie,
                    "Numero"=>$numeroSerie,
                    "Estado"=>"1"
                ];
        ...
        public function list_voucher_controller(){
            ...
            while ($reg=$query->fetch()) {
                $data[]=array( 
                    ...
                    '<div class="btn-group"><button title="Actualizar" class="btn btn-default border-info btn-sm edit" id="'.mainModel::encryption($reg['comprobante_id']).'">
                    <i class="fa fa-edit fa-xs"></i>
                    </button>'.' '.' 
                    <button type="button" title="Desactivar" class="btn btn-default border-warning btn-sm desactivate" id="'.mainModel::encryption($reg['comprobante_id']).'">
                        <i class="fa fa-ban fa-xs"></i>
                    </button>'.' '.' 
                    <button type="button" title="Eliminar" class="btn btn-default border-danger btn-sm delete" id="'.mainModel::encryption($reg['comprobante_id']).'">
                        <i class="fa fa-trash fa-xs"></i>
                    </button></div>':
                    '<div class="btn-group"><button title="Actualizar" class="btn btn-default border-info btn-sm edit" id="'.mainModel::encryption($reg['comprobante_id']).'">
                    <i class="fa fa-edit fa-xs"></i>
                    </button>'.' '.' 
                    <button type="button" title="Activar" class="btn btn-default border-primary btn-sm activate" id="'.mainModel::encryption($reg['comprobante_id']).'">
                    ...
                    "3"=>($reg['comprobante_estado'])?'<span class="badge bg-green-400">Activado</span>':'<span class="badge bg-danger-400">Desactivado</span>' 
                );
            }
            $results=array(
                     "sEcho"=>1,
