<?php
    ...
    class usuarioControllers extends usuarioModels{
        ...
        public function update_perfil_controller(){
            ...
            $celular=mainModel::clean_chain($_POST['celular']);
            $profesion=mainModel::clean_chain($_POST['profesion']);
            $login=mainModel::clean_chain($_POST['usuario']);
            $descripcion=mainModel::clean_chain($_POST['descripcion']);
            ...
            if (usuarioModels::update_perfil_model($dataUser)) {
                ...
                $_SESSION['birthdate_str']=$fnacimiento;
                $_SESSION['dni_str']=$dni;
        ...
        public function list_user_controller(){
            ...
            $query = usuarioModels::list_user_model($code);
            $data=Array();
            while ($reg=$query->fetch()) {
                $data[]=array(
                    "0"=>($reg['usuario_estado'])?
                    '<div class="btn-group"><button class="btn btn-default border-info btn-sm edit"   title="Actualizar" id="'.mainModel::encryption($reg['usuario_id']).'">
                    <i class="fa fa-edit fa-xs"></i></button>'.' '.'
                    <button type="button" class="btn btn-default border-danger btn-sm delete"   title="Eliminar" id="'.mainModel::encryption($reg['usuario_id']).'">
                    <i class="fa fa-trash fa-xs"></i>
                    </button></div>
                    ':'
                    <div class="btn-group"><button type="button" class="btn btn-default border-danger btn-sm delete"   title="Eliminar" id="'.mainModel::encryption($reg['usuario_id']).'">
                    <i class="fa fa-trash fa-xs"></i>
                    </button></div>',
                    "1"=>$reg['usuario_dni'],
                    "2"=>$reg['usuario_nombre']." ".$reg['usuario_apellido'],
                    "3"=>date("d/m/Y", strtotime($reg['usuario_fechanacimiento'])),
