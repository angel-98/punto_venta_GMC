<?php
    ...
    class productoControllers extends productoModels{
        ...
        public function add_prod_controller(){
            ...
            $porcentaje_prod=mainModel::clean_chain($_POST['porcentaje_prod']);
            ...
            $imagen="../Assets/images/producto/medicamento.png";
                ...
                $query=mainModel::run_simple_query("SELECT prod_codigo FROM producto WHERE prod_codigo='$barras'");
                if($query->rowCount()==1){
                    $alert=[
                        "Alert"=>"simple",
                    ...
                    $query1=mainModel::run_simple_query("SELECT prod_id FROM producto");
            ...
            while ($reg=$query->fetch()) {
                ...
                while ($obj=$query2->fetch()) {
                    ...
                    if($obj['lote_stock'] >= 15):
                        $status = '<span class="badge bg-green-400">Abastecido</span>';
                    elseif($obj['lote_stock'] > 0):
                        $status = '<span class="badge bg-warning-400">Abastecer</span>';
                    else:
                        $status = '<span class="badge bg-danger-400">Agotado</span>';
                    endif; 
                }
                $data[]=array(
                    "0"=>($stock > 0)? '<div class="btn-group">
                    <button type="button" class="btn btn-default border-primary btn-sm editarImagen" title="Actualizar imagen" data-toggle="modal" 
                    id="'.mainModel::encryption($reg['prod_id']).'">
                    <i class="fa fa-image fa-xs"></i>
                    </button>'.' '.' 
                    <button type="button" class="btn btn-default border-green btn-sm addLote" title="Aregar lote existente" data-toggle="modal" id="'.mainModel::encryption($reg['prod_id']).'">
                    <i class="fa fa-plus-square fa-xs"></i>
                    </button>'.' '.' 
                    <button type="button"   title="Actualizar" class="btn btn-default border-info btn-sm edit" id="'.mainModel::encryption($reg['prod_id']).'">
                    ...
                    <button type="button" class="btn btn-default border-green btn-sm addLote" data-toggle="modal" id="'.mainModel::encryption($reg['prod_id']).'">
                    <i class="fa fa-plus-square fa-xs"></i>
                    </button>'.' '.' 
                    <button type="button"   title="Actualizar" class="btn btn-default border-info btn-sm edit" id="'.mainModel::encryption($reg['prod_id']).'">
                    <i class="fa fa-edit fa-xs"></i>
                    </button>'.' '.' 
                    <button type="button"   title="Eliminar" class="btn btn-default border-danger btn-sm delete" id="'.mainModel::encryption($reg['prod_id']).'">
                    <i class="fa fa-trash fa-xs"></i>
                    </button></div>',
                    "1"=>strtoupper($reg['prod_nombre']),
                    "2"=>strtoupper($reg['prod_concentracion']),
                    "3"=>strtoupper($reg['prod_adicional']),
                    "4"=>$stock ,
                    "5"=>formatMoney($reg['prod_precioV']),
                    "6"=>strtoupper($reg['lab_nombre']), 
                    "7"=>strtoupper($reg['tipo_nombre']),
                    "8"=>strtoupper($reg['present_nombre']),
                    "9"=>$status
                );
            }
            $results=array(
