<?php
    ...
    if(!isset($_SESSION['token_str'])){
        ...
        if ($_SESSION['type_str']=="Administrador") 
        {
            ...
            class PDF extends FPDF
            {
                ...
                function Header()
                {
                    if ($this->page == 1)
                    {
                        ...
                        $t1 = date("d/m/Y", strtotime($_GET['fecha1']));
                        $t2 = date("d/m/Y", strtotime($_GET['fecha2']));

                        $this->Cell(115,10,'COMPRAS  ENTRE EL '.$t1.' Y '.$t2,0,0,'C');
            ...
            $listado = $compras->compra_fecha($fecha1,$fecha2);
            ...
            try {
                ...
                $pdf->Cell(90,10,'PROVEEDOR',0,0,'L',1);
                ...
                if (is_array($listado) || is_object($listado))
                {
                    foreach ($listado as $row => $column) {
                        ...
                        $productos = $compras->cantidad_productos_model($column["compra_id"]);
                        foreach ($productos as $row => $quanty) {
                            $cantidad = $quanty["total"];
                        }
                        if($column["compra_estado"]== "1"){
                            $estado = "Pagada";
                        }else{
                            $estado = "Anulada";
                        }
                        ...
                        $pdf->Cell(35,5,$column["compra_serie"].'-'.$column["compra_numComprobante"],0,0,'C',1);
                        $pdf->Cell(35,5,$column["compra_tipoComprobante"],0,0,'C',1);
                        $pdf->Cell(35,5,formatMoney($column["compra_impuesto"]),0,0,'C',1);
                        $pdf->Cell(40,5, date("d/m/Y", strtotime($column["compra_fecha"])),0,0,'C',1);
                        $pdf->Cell(90,5,$column["proved_nombre"],0,0,'L',1);
                        $pdf->Cell(25,5,$cantidad,0,0,'C',1);
                        $pdf->Cell(22,5,formatMoney($column["compra_total"]),0,0,'C',1);
                        $pdf->Cell(30,5,$estado,0,0,'C',1);
                        $total = $total + $column["compra_total"];
                        $pdf->Ln(6);
                        $get_Y = $pdf->GetY();
                    }
                    ...
                    $pdf->Text(10,$get_Y + 10,'MONTO TOTAL  ENTRE DICHAS FECHAS : '.formatMoney($total));
                }
                $pdf->Output('I','Compras_del_'.$fecha1.'_al_'.$fecha2.'.pdf');
